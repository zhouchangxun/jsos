// 说明：
// 1. 每个文件系统节点都有一个唯一的名称，用于标识文件或目录。
// 2. 文件系统节点可以是目录或文件。
// 3. 目录节点可以包含子节点，而文件节点则没有子节点。
// 4. 文件系统节点的路径是从根目录开始的，每个目录之间用斜杠分隔。
// 5. 文件系统节点的路径可以是绝对路径或相对路径。
// 6. 绝对路径以斜杠开头，相对路径不以斜杠开头。
// 7. 根目录的路径为 "/"。
// 8. 当前工作目录的路径存储在 `currentPath` 属性中。(多进程时每个进程有独立的当前工作目录)
// 9. 文件系统节点的属性包括名称、类型、子节点等。
// 10. 文件系统节点的操作包括创建、删除、重命名、移动等。
// 11. 文件系统节点的状态包括是否存在、是否可读、是否可写等。
